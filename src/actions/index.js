...
export const contractsInit = (userConnect) => async (dispatch) => {
  ...
  try {
    ...
    if (accounts.length !== 0) {
      dispatch(
        setAlert("info", "Connecting authorized account: " + accounts[0])
      );
      ...
      data.day365 = new ethers.Contract(
        process.env.REACT_APP_CONTRACT_ADDRESS,
        nftContractJSON.abi,
        signer
      );
      data.auctioner = new ethers.Contract(
        process.env.REACT_APP_AUCTIONER_ADDRESS,
        auctionerContractJSON.abi,
        signer
      );
      data.currentFee = Number.parseFloat(
        ethers.utils.formatEther(await data.day365.currentFee())
      );
...
export const nftInit = (nftId) => async (dispatch, getState) => {
  ...
  if (switchUpdate || !nftsInitial[nftId - 1]) {
    ...
    if (file) {
      ...
      data.location = file.data.attributes.filter(
        (a) => a.trait_type === "location"
      )[0].value;
      data.temperature = file.data.attributes.filter(
        (a) => a.trait_type === "temperature"
      )[0].value;
    }
...
const auctionInit = (nftId, isUpdate) => async (dispatch, getState) => {
  ...
  if (isUpdate || switchUpdate || !nftState.auction.wasInit) {
    ...
    const winner = auction[3]
      ? auction[3].toLowerCase()
      : ethers.constants.AddressZero;
    ...
    (account.toLowerCase() === nftState.auction.owner &&
      !nftState.auction.isOwner) ||
    (account.toLowerCase() === nftState.auction.winner &&
      !nftState.auction.isWinner)
  ) {
    dispatch({
      type: AUCTION_UPDATE,
      payload: {
...
export const startAuction = (nftId, startPrice, daysEnd, callback) => async (
  dispatch,
  getState
) => {
  const currentNftId = Number.parseInt(nftId);
  ...
  try {
    dispatch({
      type: CONTRACTS_UPDATE,
      payload: { auctionLoading: true },
    });

    let approvalEvent = contract.filters.Approval(
      account,
      auctioner.address,
      currentNftId
    );
...
export const cancelAuction = (nftId) => async (dispatch, getState) => {
  ...
  try {
    dispatch({
      type: NFT_UPDATE,
      payload: { id: currentNftId, isLoading: true },
    });
    ...
    dispatch({
      type: NFT_UPDATE,
      payload: { id: currentNftId, isLoading: false },
    });
    dispatch(setAlert("danger", `Cancel auction failed for nftId: ${nftId}`));
  }
};

export const bidAuction = (nftId, price, callback) => async (
  dispatch,
  getState
) => {
  const currentNftId = Number.parseInt(nftId);
  ...
  try {
    ...
    await auctioner.bid(currentNftId, {
...
export const setCurrentNft = (nftId) => async (dispatch, getState) => {
  ...
  if (nftId > 0) {
    dispatch({
      type: CONTRACTS_UPDATE,
      payload: { currentNftId: Number.parseInt(nftId) },
    });
  } else {
    dispatch({
      type: NFT_UPDATE,
